    ...
    >
      ...
      >
        ...
        <div className="relative" style={{ transformStyle: 'preserve-3d' }}>
           ...
           >
              ...
              {[...Array(8)].map((_, i) => (
                <div 
                  key={i}
                  className="absolute inset-0 bg-accent-orange rounded-[2.5rem] border border-white/10 flex items-center justify-center shadow-2xl"
                  style={{ 
                ...
                >
                  {i === 0 && <Zap size={80} className="text-white fill-white drop-shadow-[0_0_20px_rgba(255,255,255,0.5)]" />}
                </div>
              ))}
              
              {/* Inner Core Light */}
              <div className="absolute inset-0 bg-white/20 rounded-[2.5rem] blur-xl animate-pulse translate-z-[15px]" />
        ...
        <div className="flex flex-col items-center gap-4 text-center">
           <motion.h1 
             className="text-5xl md:text-6xl font-heading font-black tracking-[-0.08em] text-white uppercase"
        ...
        <div className="w-full space-y-8" style={{ transformStyle: 'preserve-3d', transform: 'rotateX(5deg)' }}>
          <div className="h-2 w-full bg-white/[0.02] border border-white/5 rounded-full relative overflow-hidden shadow-[inset_0_2px_4px_rgba(0,0,0,0.5)]">
            <motion.div 
          ...
          <div className="grid grid-cols-3 gap-6">
             {[
               ...
               { label: 'Encryption', value: 'AES-256', color: 'text-white/60' },
               { label: 'Uptime', value: '99.99%', color: 'text-accent-orange' }
             ].map((stat, i) => (
               <div key={i} className="flex flex-col gap-1 p-4 bg-white/[0.01] border border-white/5 rounded-2xl hover:bg-white/[0.03] transition-colors group">
                  <span className="text-[7px] font-black uppercase tracking-widest text-white/20 group-hover:text-white/40 transition-colors">{stat.label}</span>
