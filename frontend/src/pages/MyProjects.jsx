  ...
  <Card className="relative overflow-hidden group">
    ...
    <CardContent className="p-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="text-[10px] uppercase tracking-widest text-black/40 dark:text-white/40 mb-1 font-bold">{title}</p>
...
const MyProjects = () => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground p-6 md:p-10 lg:p-12 space-y-12">
      {/* Header Area */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-black/5 dark:border-white/5 pb-12">
        <div>
          <h1 className="text-5xl font-bold uppercase tracking-tighter leading-none mb-4">
            Project <span className="text-black/20 dark:text-white/20">Portfolio</span>
          </h1>
          <p className="text-sm text-black/40 dark:text-white/40 max-w-xl leading-relaxed uppercase tracking-widest font-medium">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-4">
           <h3 className="text-[10px] uppercase tracking-[0.3em] font-bold text-black/30 dark:text-white/30">Asset Inventory Control</h3>
           <span className="text-[10px] uppercase tracking-widest text-black/20 dark:text-white/20">{projects.length} Nodes Found</span>
        </div>

        <div className="grid grid-cols-1 gap-6">
          {projects.length === 0 ? (
            <Card className="border-dashed py-20 flex flex-col items-center justify-center text-center bg-black/[0.02] dark:bg-white/[0.01]">
              <div className="w-16 h-16 rounded-full bg-black/5 dark:bg-black/5 dark:bg-white/5 flex items-center justify-center mb-6">
                <Building2 className="text-black/20 dark:text-white/20" size={32} />
              </div>
              <h4 className="text-xl font-bold uppercase tracking-tight mb-2 text-black/60 dark:text-white/60">No properties listed yet</h4>
              <p className="text-xs text-black/30 dark:text-white/30 uppercase tracking-widest mb-8">Start your journey by tokenizing your first asset</p>
            ...
            projects.map((project) => (
              <motion.div 
                ...
                viewport={{ once: true }}
              >
                <Card className="hover:border-black/20 dark:border-white/20 transition-colors bg-background group overflow-hidden">
                  <div className="flex flex-col lg:flex-row divide-y lg:divide-y-0 lg:divide-x divide-white/5">
                    ...
                    <div className="lg:w-1/3 p-6 flex flex-col justify-between">
                       ...
                       <div className="mt-8">
                          <div className="flex justify-between text-[10px] uppercase tracking-widest text-black/40 dark:text-white/40 mb-2">
                          ...
                          <div className="h-1.5 w-full bg-black/5 dark:bg-black/5 dark:bg-white/5 rounded-none overflow-hidden">
                             <motion.div 
                               initial={{ width: 0 }}
                    ...
                    {/* Financial Nodes */}
                    <div className="lg:w-1/4 p-6 bg-black/[0.02] dark:bg-white/[0.01]">
                       <h5 className="text-[9px] uppercase tracking-[0.2em] text-black/20 dark:text-white/20 mb-4 font-bold">Token Metrics</h5>
                       <div className="space-y-4">
                          <div className="flex justify-between items-baseline">
                             <span className="text-[10px] uppercase text-black/30 dark:text-white/30">Total Bricks</span>
                             <span className="text-sm font-bold font-mono">{project.financial?.total_bricks?.toLocaleString()}</span>
                          </div>
                          <div className="flex justify-between items-baseline">
                             <span className="text-[10px] uppercase text-black/30 dark:text-white/30">IPO Price</span>
                             <span className="text-sm font-bold font-mono text-green-500">₹{project.financial?.ipo_price}</span>
                          </div>
                          <div className="flex justify-between items-baseline pt-4 border-t border-black/5 dark:border-white/5">
                             <span className="text-[10px] uppercase text-black/30 dark:text-white/30 font-bold">Valuation</span>
                    ...
                    <div className="lg:flex-1 p-6 flex flex-col justify-between items-end">
                       <div className="flex gap-2 mb-6">
                        ...
                        <div className="flex flex-wrap gap-2 justify-end w-full">
                          <Button 
                            variant="outline" 
                            className="text-[10px] h-10 px-4 tracking-widest rounded-none border-black/5 dark:border-white/5 hover:bg-black/5 dark:bg-black/5 dark:bg-white/5"
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-sm p-4 animate-in fade-in duration-300">
           <Card className="max-w-md w-full bg-background border-black/10 dark:border-white/10 shadow-2xl">
              <CardHeader className="border-b border-black/5 dark:border-white/5">
                ...
                <p className="text-[10px] uppercase tracking-widest text-black/40 dark:text-white/40 mt-1">Project: {selectedProject?.title}</p>
              </CardHeader>
              <CardContent className="p-6">
                <form onSubmit={handleDepositRevenue} className="space-y-6">
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                         <label className="text-[10px] uppercase tracking-widest text-black/40 dark:text-white/40">Month</label>
                         <select 
                           className="w-full bg-black/5 dark:bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 p-3 text-sm text-foreground focus:border-primary-500 outline-none"
                      ...
                      <div className="space-y-2">
                         <label className="text-[10px] uppercase tracking-widest text-black/40 dark:text-white/40">Year</label>
                         <Input 
                           type="number"
