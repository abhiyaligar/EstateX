...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-surface/30 pb-20 relative overflow-x-hidden">
        ...
        {showFullAudit && (
          <>
            ...
            <motion.div 
              ...
              className="fixed top-0 right-0 h-screen w-full max-w-[600px] z-[90] bg-background border-l border-black/10 dark:border-white/10 shadow-[0_0_50px_rgba(0,0,0,1)] p-8 md:p-12 overflow-y-auto"
            >
               <div className="flex justify-between items-center mb-16">
                  <div className="space-y-1">
                     <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[#D4AF37]">Exhaustive Audit</p>
                     <h2 className="text-2xl md:text-3xl font-bold tracking-tighter uppercase">Ledger <span className="text-black/20 dark:text-white/20">Flow</span></h2>
                  </div>
                  <button onClick={() => setShowFullAudit(false)} className="p-3 text-zinc-600 hover:text-foreground transition-colors"><X size={24} /></button>
               </div>
               <div className="space-y-6">
                  {paginatedEvents.map((event, i) => (
                    <div key={i} className="group flex flex-col gap-2 p-6 border border-black/5 dark:border-white/5 bg-black/[0.02] dark:bg-white/[0.01] hover:bg-black/[0.04] dark:bg-white/[0.02] transition-all">
                       ...
                       <div className="flex justify-between items-end">
                          ...
                          <span className={`text-base font-black tracking-tighter ${event.isPositive ? 'text-[#D4AF37]' : 'text-foreground'}`}>{event.amount}</span>
                       </div>
                    </div>
                  ))}
               </div>
      ...
      {isKYCPending && (
        <div className="bg-surface/10 border-b border-[#D4AF37]/20 py-3 px-6 flex items-center justify-center gap-3 text-center">
      ...
      <div className="max-w-[1600px] mx-auto px-6 md:px-12 pt-8 md:pt-16">
        <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-10 md:gap-12 mb-12 md:mb-16">
          ...
          <div className="flex flex-col sm:flex-row gap-8 md:gap-20">
             ...
             <div className="space-y-1 md:space-y-2 border-t border-black/5 dark:border-white/5 pt-4 sm:border-0 sm:pt-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-20 border-t border-black/5 dark:border-white/5 pt-10 md:pt-12">
          {/* Main Column */}
          <div className="lg:col-span-2 space-y-12 md:space-y-16">
            <div className="space-y-8">
              <div className="flex justify-between items-center">
                 <h3 className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-zinc-600 dark:text-zinc-600 dark:text-zinc-400">Portfolio Performance</h3>
                 <div className="flex gap-4 text-[8px] md:text-[9px] font-black uppercase tracking-widest text-zinc-700">
                    ...
                    <button className="text-foreground border-b border-[#D4AF37] pb-1">1Y</button>
              ...
              <div className="relative h-[250px] md:h-[400px] w-full min-w-0 group">
                <div className="absolute top-4 right-4 md:right-10 z-20 bg-surface text-black px-2 md:px-3 py-1 md:py-1.5 text-[8px] md:text-[9px] font-black uppercase tracking-widest shadow-2xl">
                ...
                <ResponsiveContainer width="99%" height="99%" minWidth={1} minHeight={1}>
                  <AreaChart data={chartData}>
                    ...
                    <YAxis hide />
                    <Tooltip 
                      contentStyle={{ backgroundColor: '#000', border: '1px solid rgba(212,175,55,0.2)', borderRadius: '0px' }} 
                      itemStyle={{ color: '#D4AF37', fontSize: '9px', fontWeight: 900, textTransform: 'uppercase' }} 
                      labelStyle={{ color: '#ffffff', fontSize: '10px', textTransform: 'uppercase', opacity: 0.6 }} 
                    />
            ...
            <div className="space-y-8 md:space-y-10 pt-4 md:pt-10">
               <div className="flex justify-between items-center">
                  <h3 className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-zinc-600 dark:text-zinc-600 dark:text-zinc-400">Sovereign Bricks Portfolio</h3>
               ...
               {dashboardData?.portfolio?.length > 0 ? (
                 <div className="space-y-3">
                    {dashboardData.portfolio.map((item, i) => (
                      <motion.div 
                        ...
                        key={i} 
                        className="group flex flex-col md:flex-row md:items-center justify-between p-4 md:p-6 bg-black/[0.02] dark:bg-white/[0.01] border border-black/5 dark:border-white/5 hover:border-[#D4AF37]/30 transition-all cursor-pointer gap-6 md:gap-0"
                      >
                         <div className="flex items-center gap-4 md:gap-8 flex-1 min-w-0">
                            {/* Small Square Asset Node */}
                            <div className="w-12 h-12 md:w-16 md:h-16 shrink-0 bg-surface border border-black/10 dark:border-white/10 overflow-hidden">
                               <img src={item.image_url} alt={item.project?.title} className="w-full h-full object-cover transition-all duration-700" />
                            </div>
                            
                            <div className="min-w-0 flex-1 lg:flex lg:items-center lg:gap-20">
                               <div className="min-w-0 md:min-w-[200px] lg:min-w-[250px]">
                                  <h4 className="text-sm md:text-base font-bold text-foreground group-hover:text-[#D4AF37] transition-colors truncate uppercase tracking-tight">{item.project?.title || 'Sovereign Asset'}</h4>
                                  <p className="text-[9px] text-zinc-600 truncate uppercase tracking-widest font-black mt-0.5 flex items-center gap-2">
                                     <MapPin size={9} className="text-zinc-800 dark:text-zinc-200" /> {item.display_location}
                                  </p>
                               </div>
                         ...
                         <div className="shrink-0 w-full md:w-auto">
                            <Link to={`/properties/${item.project?.id}`} className="block">
                               <button className="w-full md:w-auto px-6 py-3 md:py-2 bg-white text-black text-[9px] font-black uppercase tracking-[0.2em] hover:bg-surface transition-all">
                                  Audit Node
                               </button>
                            </Link>
                         </div>
                 ...
                 <div className="py-20 border border-black/5 dark:border-white/5 bg-black/[0.02] dark:bg-white/[0.01] flex flex-col items-center justify-center gap-4 text-center px-6">
                    <Briefcase size={28} className="text-zinc-800 dark:text-zinc-200" />
          ...
          {/* Right Column */}
          <div className="space-y-12 md:space-y-16 lg:border-l lg:border-black/5 dark:border-white/5 lg:pl-12 xl:pl-20">
            <div className="space-y-10">
              <div className="flex justify-between items-center">
                 <h3 className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-zinc-600 dark:text-zinc-600 dark:text-zinc-400">Ledger Events</h3>
                 <button onClick={() => setShowFullAudit(true)} className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-zinc-700 hover:text-foreground transition-colors flex items-center gap-2">Audit <ChevronRight size={10} /></button>
              </div>
              <div className="space-y-8 md:space-y-10">
                 {allLedgerEvents.slice(0, 5).map((event, i) => (
                   <div key={i} className="group relative flex flex-col gap-1 md:gap-2 pb-6 md:pb-8 border-b border-black/5 dark:border-white/5 last:border-0">
                      ...
                      <div className="flex justify-between items-end">
                         <h5 className="text-xs md:text-sm font-bold text-black/90 dark:text-white/90 max-w-[180px] md:max-w-[200px] leading-snug">{event.title}</h5>
                         <span className={`text-xs md:text-sm font-black tracking-tighter ${event.isPositive ? 'text-[#D4AF37]' : 'text-foreground'}`}>{event.amount}</span>
                      </div>
                   </div>
                 ))}
              </div>
            </div>
            <div className="bg-black/[0.02] dark:bg-white/[0.01] md:bg-black/[0.04] dark:bg-white/[0.02] border border-black/5 dark:border-white/5 p-6 md:p-8 space-y-6">
               ...
               {isKYCPending && <div className="p-4 bg-surface/5 border border-[#D4AF37]/10 rounded-none space-y-2"><p className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-[#D4AF37]">Identity Pending</p><p className="text-[10px] md:text-[11px] text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 leading-relaxed font-medium">Sovereign trading permissions are currently restricted awaiting manual audit.</p></div>}
               <div className="p-4 bg-black/5 dark:bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-none space-y-2 opacity-50"><p className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-zinc-600 dark:text-zinc-400">Node Sync</p><p className="text-[10px] md:text-[11px] text-zinc-600 leading-relaxed font-medium">Connected to high-performance private ledger node.</p></div>
            </div>
          </div>
        </div>
      </div>
