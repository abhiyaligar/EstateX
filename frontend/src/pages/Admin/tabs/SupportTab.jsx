...
const SupportTab = ({ addToast }) => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      default: return <span className="px-2 py-0.5 bg-zinc-500/10 text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 text-[8px] font-black uppercase tracking-widest border border-zinc-500/20">Closed</span>;
    }
  };
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <p className="text-[10px] font-bold text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 uppercase tracking-widest">Managing {total} Inquiry Nodes</p>
        </div>
        <div className="flex gap-4 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
          <div className="flex bg-background border border-black/5 dark:border-white/5 p-1 shrink-0">
            {['', 'open', 'in_progress', 'resolved'].map((status) => (
              <button
                key={status}
                onClick={() => setStatusFilter(status)}
                className={`px-4 py-1.5 text-[9px] font-black uppercase tracking-widest transition-all ${
                  statusFilter === status ? 'bg-surface text-black' : 'text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 hover:text-foreground'
                }`}
              >
                {status || 'All Nodes'}
              </button>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 space-y-4">
          {loading ? (
            ...
            <div className="py-20 text-center border border-dashed border-black/5 dark:border-white/5 text-zinc-700 uppercase text-[10px] tracking-widest bg-black/[0.02] dark:bg-white/[0.01]">Zero active anomalies detected</div>
          ) : (
            tickets.map((ticket) => (
              <Card 
                key={ticket.id} 
                noPadding
                className={`cursor-pointer transition-all border-l-2 ${selectedTicket?.id === ticket.id ? 'border-l-[#C5A059] bg-surface' : 'border-l-transparent hover:border-black/10 dark:border-white/10'}`}
                onClick={() => {
                  ...
                  <div className="flex justify-between items-start">
                    <div className="space-y-4 flex-1">
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <span className="h-1 w-1 rounded-full bg-zinc-200 dark:bg-zinc-800"></span>
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 mt-2 line-clamp-1 italic">{ticket.description}</p>
                      </div>
                      <div className="flex items-center gap-4 text-[9px] font-bold text-zinc-600 uppercase tracking-widest pt-2">
                         ...
                         <div className="flex items-center gap-1.5">
                            ...
                            <span className="lowercase font-normal text-zinc-600 dark:text-zinc-600 dark:text-zinc-400">{ticket.user_email}</span>
                         </div>
                      </div>
                    </div>
                    <div className="text-right space-y-1 hidden sm:block">
                      <p className="text-[10px] font-black text-zinc-600 dark:text-zinc-400">{new Date(ticket.created_at).toLocaleDateString()}</p>
        ...
        <div className="xl:col-span-1">
          <div className="sticky top-8 space-y-6">
            {selectedTicket ? (
              <Card noPadding className="border border-[#C5A059]/30 bg-surface shadow-2xl">
                <CardHeader className="p-6 border-b border-black/5 dark:border-white/5 space-y-0 flex-row justify-between items-center">
                ...
                <CardContent className="p-6 space-y-8">
                  <div className="space-y-4">
                    ...
                    <p className="text-[12px] text-zinc-600 dark:text-zinc-400 leading-relaxed bg-black/[0.04] dark:bg-white/[0.02] p-4 border-l border-[#C5A059]/30 italic">
                      "{selectedTicket.description}"
                    </p>
                    
                    <div className="pt-4 space-y-3">
                       <div className="flex items-center gap-3 p-3 bg-black/[0.04] dark:bg-white/[0.02] border border-black/5 dark:border-white/5">
                  ...
                  <div className="space-y-6 pt-6 border-t border-black/5 dark:border-white/5">
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        className="w-full bg-background border border-black/10 dark:border-white/10 p-4 text-[11px] focus:border-[#C5A059] outline-none transition-colors resize-none placeholder:text-zinc-800 dark:text-zinc-200"
              ...
              <div className="h-64 flex flex-col items-center justify-center border border-dashed border-black/5 dark:border-white/5 bg-black/[0.02] dark:bg-white/[0.01] p-12 text-center">
                <Search size={32} className="text-zinc-800 dark:text-zinc-200 mb-4" strokeWidth={1} />
