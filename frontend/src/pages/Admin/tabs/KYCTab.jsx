      ...
      <Card noPadding className="overflow-hidden">
        <div className="p-8 border-b border-black/5 dark:border-white/5 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-black/40 dark:text-white/40 mt-1 uppercase tracking-widest">Documents requiring manual review</p>
          </div>
              ...
              <AdminTableRow key={app.id}>
                <AdminTableCell>
                  <div className="flex flex-col">
                    ...
                    <span className="text-[9px] text-black/30 dark:text-white/30 font-mono">NODE_UID: {app.id.substring(0,8)}</span>
                  </div>
                </AdminTableCell>
                <AdminTableCell>
                ...
                <AdminTableCell>
                   <div className="flex flex-col gap-1.5">
                     <div className="flex items-center gap-2">
                        <span className="text-[9px] text-black/20 dark:text-white/20 uppercase font-bold w-12">PAN</span>
                        <span className="text-[10px] text-black/60 dark:text-white/60 font-mono tracking-widest">{app.pan_number || '---'}</span>
                     </div>
                     <div className="flex items-center gap-2">
                        <span className="text-[9px] text-black/20 dark:text-white/20 uppercase font-bold w-12">UIDAI</span>
                        <span className="text-[10px] text-black/60 dark:text-white/60 font-mono tracking-widest">**** **** {app.aadhaar_last_4_digits || '----'}</span>
                     </div>
                   </div>
                </AdminTableCell>
                <AdminTableCell>
                   <div className="flex gap-2">
                     {app.pan_image_url && (
                       <button 
                        ...
                        className="w-9 h-9 bg-black/5 dark:bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 flex items-center justify-center hover:bg-primary-500/20 hover:border-primary-500/50 transition-all rounded-lg group"
                        title="View PAN"
                       >
                         <FileCheck size={16} className="text-black/20 dark:text-white/20 group-hover:text-primary-500" />
                       </button>
                     )}
                ...
                <AdminTableCell align="right">
                   {!app.assigned_admin_id ? (
                     ...
                     <span className="text-[9px] text-black/20 dark:text-white/20 italic uppercase tracking-[0.2em] font-bold">LOCKED BY OTHER</span>
                   )}
                </AdminTableCell>
              </AdminTableRow>
            ))}
