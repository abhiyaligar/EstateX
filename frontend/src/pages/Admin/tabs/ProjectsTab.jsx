       ...
       <Card noPadding className="overflow-hidden">
        ...
        <AdminTable>
          ...
          <tbody>
            {projects.length === 0 ? (
              <tr>
                <td colSpan="6" className="p-20 text-center text-white/20 uppercase tracking-widest text-xs italic">No projects available in the ecosystem.</td>
              </tr>
            ) : (
              projects.map(p => (
                <AdminTableRow 
                ...
                >
                  ...
                  <AdminTableCell>
                    ...
                    <span className="text-[10px] text-white/40 font-mono">ID: {p.builder_id?.substring(0,8) || 'N/A'}</span>
                  </AdminTableCell>
                  <AdminTableCell>
                  ...
                  <AdminTableCell>
                    <span className="text-xs font-bold text-white font-mono">₹{(p.financial?.total_escrow_held || 0).toLocaleString()}</span>
                  </AdminTableCell>
                  <AdminTableCell>
                  ...
                  <AdminTableCell align="right">
                      <div className="flex justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                        ...
                        {p.ipo_status === 'active' && (
                          <Button size="sm" className="text-[10px] h-9 px-4 bg-blue-600 hover:bg-blue-700 text-white font-bold" onClick={() => onIPOAction(p.id, 'complete')}>COMPLETE IPO</Button>
                        )}
                        {p.status === 'halted' ? (
                          <Button size="sm" variant="primary" className="text-[10px] h-9 px-4 bg-green-600 hover:bg-green-700 text-white font-bold" onClick={() => onHaltAction(p.id, p.status)}>RESUME</Button>
                        ) : (
