    ...
    >
       <Card noPadding className="overflow-hidden">
        <div className="p-8 border-b border-black/5 dark:border-white/5">
          ...
          <p className="text-xs text-black/40 dark:text-white/40 mt-1 uppercase tracking-widest">Control asset lifecycle and match completion</p>
        </div>
        ...
        <AdminTable>
          ...
          <tbody>
            {projects.length === 0 ? (
              <tr>
                <td colSpan="6" className="p-20 text-center text-black/20 dark:text-white/20 uppercase tracking-widest text-xs italic">No projects available in the ecosystem.</td>
              </tr>
            ) : (
              projects.map(p => (
                <AdminTableRow 
                  ...
                  <AdminTableCell>
                    <div className="flex flex-col gap-1">
                        ...
                        <div className="flex gap-2">
                            <span className={`text-[8px] uppercase font-bold tracking-widest ${p.compliance?.rera_approved ? 'text-green-500' : 'text-black/20 dark:text-white/20'}`}>RERA</span>
                            <span className={`text-[8px] uppercase font-bold tracking-widest ${p.compliance?.environmental_clearance ? 'text-green-500' : 'text-black/20 dark:text-white/20'}`}>ENV</span>
                            <span className={`text-[8px] uppercase font-bold tracking-widest ${p.compliance?.insurance_coverage ? 'text-green-500' : 'text-black/20 dark:text-white/20'}`}>INS</span>
                        </div>
                    </div>
                  </AdminTableCell>
                  <AdminTableCell>
                    <span className="block text-sm font-bold text-foreground uppercase">{p.builder?.company_name || 'UNKNOWN'}</span>
                    <span className="text-[10px] text-black/40 dark:text-white/40 font-mono">ID: {p.builder_id?.substring(0,8) || 'N/A'}</span>
                  </AdminTableCell>
                  <AdminTableCell>
