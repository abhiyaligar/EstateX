...
const RevenueSettlementModal = ({ isOpen, onClose, result }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-background/95 backdrop-blur-md p-4">
       <motion.div 
         initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
         className="max-w-2xl w-full bg-background border border-green-500/20 shadow-[0_0_50px_-12px_rgba(34,197,94,0.3)]"
       >
          <div className="p-8 border-b border-border flex justify-between items-center bg-green-500/5">
             <div>
                <div className="flex items-center gap-2 text-green-500 mb-1">
                   <CheckCircle2 size={18} />
                   <h3 className="text-xl font-bold uppercase tracking-tighter">Settlement Successful</h3>
                </div>
                <p className="text-[10px] uppercase tracking-widest text-foreground/40">Distribution cycle executed across node network</p>
             </div>
             <button onClick={onClose} className="text-foreground/20 hover:text-foreground"><X size={20}/></button>
          </div>
          <div className="p-8 space-y-6">
             <div className="grid grid-cols-3 gap-4">
                <div className="p-4 bg-foreground/5 border border-border">
                   <p className="text-[8px] uppercase tracking-widest text-foreground/30 mb-1">Gross Yield</p>
                   <p className="text-lg font-mono font-bold text-foreground">₹{parseFloat(result.cycle.gross_amount).toLocaleString()}</p>
                </div>
                <div className="p-4 bg-foreground/5 border border-border">
                   <p className="text-[8px] uppercase tracking-widest text-foreground/30 mb-1">Platform Fee (1%)</p>
                   <p className="text-lg font-mono font-bold text-amber-500">₹{parseFloat(result.cycle.fee_amount).toLocaleString()}</p>
                </div>
                <div className="p-4 bg-foreground/5 border border-border">
                   <p className="text-[8px] uppercase tracking-widest text-foreground/30 mb-1">Total Payouts</p>
                   <p className="text-lg font-mono font-bold text-green-500">{result.payouts.length}</p>
                </div>
             </div>

             <div>
                <h4 className="text-[10px] uppercase tracking-[0.2em] font-bold text-foreground/30 mb-4">Credited Investors (Mature Holdings Only)</h4>
                <div className="max-h-[300px] overflow-y-auto border border-border">
                   <table className="w-full text-left text-[10px]">
                      <thead className="bg-foreground/5 sticky top-0">
                         <tr className="uppercase tracking-widest text-foreground/40 border-b border-border">
                      ...
                      <tbody className="divide-y divide-white/5">
                         {result.payouts.map((p, i) => (
                           <tr key={i} className="hover:bg-foreground/[0.04]">
                              <td className="p-4">
                                 <span className="block font-bold text-foreground uppercase">{p[1]} {p[2]}</span>
                                 <span className="text-foreground/30 lowercase">{p[3]}</span>
                              </td>
