      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] uppercase tracking-widest text-black/40 dark:text-white/40">Critical Protocol Execution</p>
          </div>
        </div>

        <p className="text-sm text-black/60 dark:text-white/60 leading-relaxed">{message}</p>
...
export const AdminToast = ({ message, type = 'success', onClose }) => {
  ...
  return (
    <motion.div 
      ...
      className="fixed bottom-10 right-10 z-[300] bg-background border border-black/10 dark:border-white/10 p-4 pl-6 pr-10 flex items-center gap-4 shadow-2xl min-w-[300px]"
    >
      {icons[type]}
      <span className="text-xs font-bold uppercase tracking-widest">{message}</span>
      <button onClick={onClose} className="absolute top-2 right-2 text-black/20 dark:text-white/20 hover:text-foreground"><X size={14}/></button>
    </motion.div>
  );
};
