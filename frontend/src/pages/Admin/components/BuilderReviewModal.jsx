        ...
        <CardContent className="overflow-y-auto p-8 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <h4 className="text-[10px] uppercase tracking-[0.2em] text-foreground/40 font-bold border-b border-border pb-2">Business Identity</h4>
              <div className="space-y-3">
                {[
                  ...
                  <div key={item.label} className="flex justify-between text-xs">
                    <span className="text-foreground/20 uppercase">{item.label}</span>
                    <span className="font-mono text-foreground/80">{item.value || 'N/A'}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h4 className="text-[10px] uppercase tracking-[0.2em] text-foreground/40 font-bold border-b border-border pb-2">Compliance Matrix</h4>
              <div className="space-y-3">
                {[
          ...
          <div className="space-y-4">
            <h4 className="text-[10px] uppercase tracking-[0.2em] text-foreground/40 font-bold border-b border-border pb-2">Digital Asset Board</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                ...
                <div key={doc.label} className="p-4 bg-foreground/[0.04] border border-border flex flex-col justify-between">
                  <span className="text-[9px] uppercase tracking-widest text-foreground/40 mb-3">{doc.label}</span>
                  {doc.url ? (
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      className="w-full text-[9px] h-8 border border-border"
                      onClick={() => window.open(doc.url, '_blank')}
                    >
                      VIEW RAW ASSET
                    </Button>
                  ) : (
                    <span className="text-[9px] text-foreground/10 uppercase italic">Not Provided</span>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="p-6 bg-foreground/[0.04] border border-border">
            <h4 className="text-[10px] uppercase tracking-[0.2em] text-foreground/40 font-bold mb-4">Financial Settlement Node</h4>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-[10px] uppercase tracking-widest">
              <div>
                <span className="block text-foreground/20 mb-1">Bank</span>
                <span className="text-foreground font-bold">{builder.bank_name || 'N/A'}</span>
              </div>
              <div>
                <span className="block text-foreground/20 mb-1">Account</span>
                <span className="text-foreground font-bold font-mono">{builder.bank_account_number || 'N/A'}</span>
              </div>
              <div>
                <span className="block text-foreground/20 mb-1">Beneficiary</span>
                <span className="text-foreground font-bold">{builder.bank_account_name || 'N/A'}</span>
              </div>
              <div>
                <span className="block text-foreground/20 mb-1">IFSC</span>
          ...
          <div className="pt-8 border-t border-border space-y-6">
            <div className="space-y-3">
              <label className="text-[10px] uppercase tracking-[0.3em] font-bold text-foreground/40">Audit Comments / Rejection Reasoning</label>
              <textarea 
                className="w-full bg-background border border-border p-4 text-xs font-mono text-foreground focus:border-primary-500 outline-none min-h-[100px]"
