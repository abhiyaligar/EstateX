...
const OrderBookRow = ({ price, quantity, type, isHeader = false, total = 0, maxTotal = 1, onClick }) => (
  ...
  >
    ...
    <div className="flex justify-between items-center h-7 px-3 md:px-5 relative z-10">
      ...
      <span className={`text-[10px] md:text-[11px] font-mono font-bold ${isHeader ? 'text-zinc-600' : 'text-zinc-600 dark:text-zinc-400'}`}>
...
const TradeHistoryRow = ({ price, quantity, time, type }) => (
  <div className="flex items-center justify-between text-[10px] py-2 px-4 border-b border-black/5 dark:border-white/5 hover:bg-black/[0.04] dark:bg-white/[0.02] transition-colors">
     ...
     <span className="text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 font-mono">{quantity} BK</span>
...
const TradeForm = ({ 
    ...
    <form onSubmit={handlePlaceOrder} className="space-y-6">
        ...
        <div className="flex bg-surface p-1 rounded-xl border border-black/5 dark:border-white/5">
            <button 
                type="button"
                onClick={() => setOrderType('buy')} 
                className={`flex-1 py-2.5 text-[9px] font-black uppercase tracking-[0.2em] transition-all rounded-lg relative ${orderType === 'buy' ? 'text-foreground' : 'text-zinc-600 hover:text-zinc-600 dark:text-zinc-400'}`}
            >
                {orderType === 'buy' && (
            ...
            <button 
                ...
                className={`flex-1 py-2.5 text-[9px] font-black uppercase tracking-[0.2em] transition-all rounded-lg relative ${orderType === 'sell' ? 'text-foreground' : 'text-zinc-600 hover:text-zinc-600 dark:text-zinc-400'}`}
            >
                 {orderType === 'sell' && (
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-1.5">
                 <div className="flex justify-between items-end px-1">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex bg-zinc-100 dark:bg-zinc-900 rounded-md p-0.5 border border-black/5 dark:border-white/5">
                            <button 
                                type="button"
                                onClick={() => setExecutionMode('limit')}
                                className={`px-2 py-0.5 text-[7px] font-black uppercase rounded-[4px] transition-all ${executionMode === 'limit' ? 'bg-zinc-200 dark:bg-zinc-800 text-foreground' : 'text-zinc-600'}`}
                            >
                                Limit
                            </button>
                            <button 
                                ...
                                className={`px-2 py-0.5 text-[7px] font-black uppercase rounded-[4px] transition-all ${executionMode === 'market' ? 'bg-zinc-200 dark:bg-zinc-800 text-foreground' : 'text-zinc-600'}`}
                            >
                                Market
                            </button>
                        </div>
                    </div>
                    <span className="text-[9px] font-mono font-bold text-zinc-600 dark:text-zinc-400">INR</span>
                 </div>
                 <div className="relative group">
                    <input 
                        ...
                        className={`w-full bg-surface border border-black/5 dark:border-white/5 border border-black/5 dark:border-white/5 h-12 px-5 text-sm font-mono focus:border-black/20 dark:border-white/20 focus:bg-surface transition-all focus:outline-none rounded-xl text-foreground placeholder:text-zinc-600 dark:text-zinc-400 ${executionMode === 'market' ? 'opacity-50 cursor-not-allowed italic' : ''}`}
                        placeholder={executionMode === 'market' ? "BEST AVAILABLE PRICE" : "0.00"}
                    />
                    <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">
                        <TrendingUp size={14} className={`${executionMode === 'market' ? 'text-zinc-900 dark:text-zinc-100' : 'text-zinc-600 dark:text-zinc-400'} group-focus-within:text-zinc-600 transition-colors`} />
                    </div>
                 </div>
            </div>
            ...
            <div className="space-y-1.5">
                 <div className="flex justify-between items-end px-1">
                    ...
                    <span className="text-[9px] font-mono font-bold text-zinc-600 dark:text-zinc-400">BRICKS</span>
                 </div>
                 <div className="relative group">
                    <input 
                        type="number" 
                        ...
                        className="w-full bg-surface border border-black/5 dark:border-white/5 border border-black/5 dark:border-white/5 h-12 px-5 text-sm font-mono focus:border-black/20 dark:border-white/20 focus:bg-surface transition-all focus:outline-none rounded-xl text-foreground placeholder:text-zinc-600 dark:text-zinc-400"
                        placeholder="0"
                    />
                    <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">
                        <Layers size={14} className="text-zinc-600 dark:text-zinc-400 group-focus-within:text-zinc-600 transition-colors" />
                    </div>
                 </div>
                 ...
                 <div className="grid grid-cols-4 gap-1.5 mt-3">
                    {[25, 50, 75, 100].map((p) => (
                        <button
                            ...
                            className="py-2 text-[9px] font-black border border-black/5 dark:border-white/5 bg-surface border border-black/5 dark:border-white/5 hover:bg-black/5 dark:bg-black/5 dark:bg-white/5 hover:border-black/10 dark:border-white/10 text-zinc-600 hover:text-zinc-600 dark:text-zinc-400 transition-all uppercase tracking-widest rounded-lg"
                        >
                            {p === 100 ? 'MAX' : `${p}%`}
                        </button>
                    ))}
        ...
        {/* Trade Summary Card */}
        <div className="p-4 bg-surface rounded-xl border border-black/5 dark:border-white/5 border-dashed relative overflow-hidden group">
             <div className="flex items-center justify-between relative z-10">
                 ...
                 <Zap size={16} className="text-zinc-600 dark:text-zinc-400 group-hover:text-primary-500/20 transition-colors" />
             </div>
...
const PropertySearchModal = ({ projects, selectedProject, onSelect }) => {
  ...
  return (
    <>
      ...
      >
        <div className="h-10 w-10 flex items-center justify-center rounded-xl bg-black/[0.05] dark:bg-white/[0.03] border border-black/5 dark:border-white/5 group-hover:border-black/20 dark:border-white/20 group-hover:bg-black/5 dark:bg-black/5 dark:bg-white/5 transition-all relative">
          <Search size={16} className="text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 group-hover:text-foreground transition-colors relative z-10" />
          <div className="absolute inset-0 bg-primary-500/0 group-hover:bg-primary-500/10 blur-xl transition-all rounded-full" />
        </div>
        <div className="text-left">
          <p className="text-[7px] uppercase tracking-[0.4em] text-primary-600 dark:text-primary-400 font-black leading-none mb-1.5 transition-colors hidden md:block">Asset Protocol // Live</p>
          <div className="flex items-center gap-1.5">
            ...
            <ChevronDown size={10} className="text-zinc-700 group-hover:text-foreground transition-all group-hover:translate-y-0.5" />
          </div>
        </div>
      </button>
          ...
          {isOpen && (
            <div className="fixed inset-0 z-[10000] flex items-start md:items-start justify-center pt-0 md:pt-[10vh] px-0 md:px-4 pointer-events-none overflow-hidden">
              <motion.div 
                ...
                className="absolute inset-0 bg-black/40 backdrop-blur-xl md:backdrop-blur-md pointer-events-auto"
              />
              ...
              <motion.div 
                ...
                className="relative w-full md:max-w-2xl h-full md:h-auto md:max-h-[85vh] bg-surface md:bg-surface md:border md:border-black/10 dark:border-white/10 shadow-[0_0_100px_rgba(0,0,0,1)] md:rounded-2xl overflow-hidden flex flex-col pointer-events-auto"
              >
                <div className="p-5 md:p-8 border-b border-black/5 dark:border-white/5 flex items-center gap-3 md:gap-5 bg-background md:bg-black/[0.02] dark:bg-white/[0.01] shrink-0">
                  <div className="h-10 w-10 md:h-12 md:w-12 rounded-xl bg-black/[0.05] dark:bg-white/[0.03] border border-black/10 dark:border-white/10 flex items-center justify-center shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <input 
                      ...
                      className="w-full bg-transparent text-lg md:text-2xl font-black tracking-tight focus:outline-none text-foreground placeholder:text-zinc-600 uppercase"
                      autoFocus
                    />
                    <p className="text-[7px] md:text-[8px] uppercase tracking-[0.2em] md:tracking-[0.3em] text-primary-600 dark:text-primary-500 font-bold mt-0.5 truncate">Asset Discovery Protocol</p>
                  </div>
                  <button 
                      onClick={() => setIsOpen(false)} 
                      className="h-8 w-8 md:h-10 md:w-10 flex items-center justify-center rounded-full bg-black/5 dark:bg-black/5 dark:bg-white/5 text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 hover:text-foreground hover:bg-black/10 dark:bg-black/10 dark:bg-white/10 transition-all shrink-0"
                  >
                    <X size={18} />
                  </button>
                </div>

                <div className="flex-1 overflow-y-auto custom-scrollbar bg-surface md:bg-transparent">
                    ...
                    <div className="p-2 md:p-4 grid grid-cols-1 gap-2">
                      {filteredProjects.map(p => (
                        <button
                          ...
                          className={`w-full text-left p-3 md:p-4 rounded-xl flex items-center justify-between group transition-all ${selectedProject?.id === p.id ? 'bg-black/10 dark:bg-black/10 dark:bg-white/10 border border-black/10 dark:border-white/10' : 'hover:bg-black/5 dark:bg-black/5 dark:bg-white/5 border border-transparent'}`}
                        >
                          <div className="flex items-center gap-3 md:gap-5 min-w-0">
                            <div className="h-12 w-12 md:h-16 md:w-16 bg-black/5 dark:bg-white/5 rounded-xl md:rounded-2xl flex items-center justify-center border border-black/5 dark:border-white/5 group-hover:border-primary-500/30 transition-all relative overflow-hidden shrink-0">
                            ...
                            <div className="flex flex-col gap-0.5 min-w-0">
                                ...
                                <div className="flex items-center gap-2 md:gap-3 flex-wrap">
                                  <div className="flex items-center gap-1">
                                      ...
                                      <span className="text-[8px] md:text-[10px] uppercase font-black tracking-widest text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 truncate max-w-[80px] md:max-w-none">
                                  ...
                                  <div className="h-1 w-1 bg-zinc-200 dark:bg-zinc-800 rounded-full hidden md:block" />
                                  <div className="flex items-center gap-1">
                                      <span className="text-[8px] md:text-[10px] font-mono font-bold text-zinc-600 dark:text-zinc-400">₹{p.ipo_price?.toLocaleString()}</span>
                          ...
                          <div className="flex flex-col items-end gap-1 shrink-0 ml-2">
                              <div className="text-[9px] md:text-[11px] font-mono font-black text-foreground bg-black/5 dark:bg-black/5 dark:bg-white/5 px-1.5 md:px-2 py-0.5 md:py-1 rounded-lg">
                                  EX-{String(p.id).slice(0, 4).toUpperCase()}
                              </div>
                              <ChevronRight size={12} className="text-zinc-600 dark:text-zinc-400 group-hover:text-primary-500 group-hover:translate-x-1 transition-all" />
                          </div>
                        </button>
                      ))}
                    </div>
                ...
                <div className="p-4 bg-surface border-t border-black/5 dark:border-white/5 flex items-center justify-between shrink-0">
                  <div className="flex gap-2 md:gap-4 overflow-x-auto no-scrollbar">
                      <div className="flex items-center gap-1 md:gap-2 whitespace-nowrap">
                          <kbd className="px-1 md:px-1.5 py-0.5 rounded bg-zinc-200 dark:bg-zinc-800 text-[6px] md:text-[8px] font-black text-zinc-600 dark:text-zinc-400 border border-black/5 dark:border-white/5">↵</kbd>
                          <span className="text-[6px] md:text-[8px] uppercase tracking-widest text-zinc-600 font-bold">Select</span>
                      </div>
                      <div className="flex items-center gap-1 md:gap-2 whitespace-nowrap">
                          <kbd className="px-1 md:px-1.5 py-0.5 rounded bg-zinc-200 dark:bg-zinc-800 text-[6px] md:text-[8px] font-black text-zinc-600 dark:text-zinc-400 border border-black/5 dark:border-white/5">ESC</kbd>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <motion.div 
        ...
        className="bg-background border border-black/10 dark:border-white/10 w-full max-w-md p-8 shadow-2xl"
      >
        <div className="flex items-center justify-between mb-8">
           <div>
             <h3 className="text-xl font-bold uppercase tracking-tight text-foreground">Modify Intent</h3>
             <p className="text-[10px] uppercase tracking-widest text-black/30 dark:text-white/30">ID: {order.id.substring(0,8)}... ({order.order_type})</p>
           </div>
           <button onClick={onClose} className="p-2 text-foreground hover:bg-black/5 dark:bg-black/5 dark:bg-white/5 rounded-full transition-colors"><X size={20}/></button>
        </div>
        ...
        <div className="space-y-6">
           ...
           <div className="pt-6 border-t border-black/5 dark:border-white/5">
              <p className="text-[9px] text-black/20 dark:text-white/20 uppercase tracking-widest mb-4 leading-relaxed">
              ...
              <div className="flex gap-4">
                 ...
                 <Button className="flex-1 h-12 text-[10px] bg-primary-600 hover:bg-primary-700 text-foreground font-bold" onClick={() => onModify(order.id, newPrice, newQuantity)}>SUBMIT MODS</Button>
              </div>
           </div>
        </div>
      </motion.div>
...
const TradingRoom = () => {
  ...
  useEffect(() => {
    ...
    if (!mainSeriesRef.current) {
        if (chartType === 'candlestick') {
            ...
            mainSeriesRef.current = chart.addSeries(LineSeries, { color: '#c5a059', lineWidth: 1.5 });
        } else {
  ...
  if (loading && projects.length === 0) {
    return (
        <div className="flex-1 flex flex-col items-center justify-center h-full bg-surface">
  ...
  return (
    <div className="h-screen bg-background text-foreground flex flex-col font-sans overflow-hidden relative">
        ...
        <header className="h-16 md:h-20 border-b border-black/10 dark:border-white/10 px-4 md:px-8 flex items-center justify-between bg-surface/95 backdrop-blur-3xl sticky top-0 z-[110] shadow-2xl">
            <div className="flex items-center gap-3">
                <button 
                    onClick={() => navigate(-1)} 
                    className="h-10 w-10 flex items-center justify-center rounded-xl bg-black/[0.05] dark:bg-white/[0.03] border border-black/10 dark:border-white/10 text-zinc-600 dark:text-zinc-400 md:hidden"
                >
                    <ArrowLeft size={18} />
                </button>
                ...
                <div className="hidden md:block">
                        <button 
                        onClick={() => navigate(-1)} 
                        className="group flex items-center justify-center h-10 w-10 rounded-xl bg-black/5 dark:bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 hover:bg-black/10 dark:bg-black/10 dark:bg-white/10 hover:border-black/20 dark:border-white/20 transition-all text-zinc-600 dark:text-zinc-400 hover:text-foreground"
                ...
                <div className="h-8 w-px bg-black/5 dark:bg-black/5 dark:bg-white/5 hidden lg:block" />
            ...
            <div className="flex md:hidden flex-col items-end">
                ...
                {/* Desktop Metrics HUD */}
                <div className="hidden md:flex items-center gap-10 overflow-x-auto no-scrollbar ml-6 border-l border-black/5 dark:border-white/5 pl-10">
                    ...
                    <div className="space-y-1">
                        ...
                        <span className="text-[11px] font-mono font-bold text-zinc-600 dark:text-zinc-400">{volume24h.toLocaleString()} <span className="text-[9px] text-zinc-700">BK</span></span>
                    </div>
                </div>
        ...
        {/* Mobile Tab Switcher */}
        <div className="md:hidden flex bg-surface border-b border-black/5 dark:border-white/5 p-1 shrink-0">
            <button 
                onClick={() => setMobileTab('chart')}
                className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${mobileTab === 'chart' ? 'bg-black/5 dark:bg-black/5 dark:bg-white/5 text-foreground' : 'text-zinc-600'}`}
            >
                Chart
            </button>
            <button 
                onClick={() => setMobileTab('terminal')}
                className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${mobileTab === 'terminal' ? 'bg-black/5 dark:bg-black/5 dark:bg-white/5 text-foreground' : 'text-zinc-600'}`}
            >
                Terminal
            </button>
        </div>
        ...
        <div className="flex-1 h-full overflow-hidden relative">
            <AnimatePresence mode="wait">
                ...
                >
                        ...
                        {/* Left Column: Unified Technical Chart */}
                        <div className={`flex-[0_0_100%] md:flex-1 flex flex-col border-r border-black/5 dark:border-white/5 shrink-0 md:shrink-1 ${mobileTab === 'chart' ? 'flex' : 'hidden md:flex'}`}>
                            <div className="flex-1 flex flex-col h-full bg-gradient-to-b from-background to-surface/50 relative">
                                {/* Chart Header / Timeframes */}
                                <div className="h-8 border-b border-black/5 dark:border-white/5 px-4 flex items-center justify-between shrink-0 bg-surface">
                                    <div className="flex bg-black/5 dark:bg-white/5 rounded-md border border-black/5 dark:border-white/5 relative">
                                        {['1m', '5m', '1h', '1D', '1W', '1M'].map((tf) => (
                                            <button 
                                                key={tf}
                                                onClick={() => setChartTimeframe(tf)}
                                                className={`relative px-3 py-1 text-[9px] font-black tracking-tighter transition-all z-10 ${chartTimeframe === tf ? 'text-foreground' : 'text-zinc-600 hover:text-zinc-600 dark:text-zinc-400'}`}
                                            >
                                                {chartTimeframe === tf && (
                                                    <motion.div 
                                                        layoutId="activeTimeframe"
                                                        className="absolute inset-0 bg-black/10 dark:bg-black/10 dark:bg-white/10 rounded-md border border-black/5 dark:border-white/5"
                                    ...
                                    <div className="flex items-center gap-2 border-l border-black/5 dark:border-white/5 pl-4 ml-2">
                                         <button 
                                            onClick={() => setChartType('candlestick')}
                                            className={`p-1.5 rounded-md transition-all ${chartType === 'candlestick' ? 'bg-black/10 dark:bg-black/10 dark:bg-white/10 text-foreground shadow-lg' : 'text-zinc-600 hover:text-zinc-600 dark:text-zinc-400'}`}
                                         ...
                                         <button 
                                            ...
                                            className={`p-1.5 rounded-md transition-all ${chartType === 'line' ? 'bg-black/10 dark:bg-black/10 dark:bg-white/10 text-foreground shadow-lg' : 'text-zinc-600 hover:text-zinc-600 dark:text-zinc-400'}`}
                                            title="Line Chart"
                                         >
                                            <LineChart size={12} />
                                         </button>
                                         <div className="w-px h-4 bg-black/5 dark:bg-black/5 dark:bg-white/5 mx-1" />
                                         <button className="text-zinc-600 hover:text-foreground transition-colors p-1.5"><Maximize2 size={12} /></button>
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 relative overflow-hidden bg-background">
                                    ...
                                    {formattedChartData.price.length === 0 && (
                                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/60 backdrop-blur-sm z-20">
                                            <Activity size={24} className="text-primary-500 mb-2 animate-pulse" />
                                            <p className="text-[8px] uppercase tracking-[0.3em] text-primary-600 dark:text-primary-400 font-black">Syncing Market Data...</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className={`w-full md:w-80 lg:w-96 md:flex-1 md:h-full flex flex-col border-r border-black/5 dark:border-white/5 bg-background dark:bg-black/20 shrink-0 ${mobileTab === 'terminal' ? 'flex' : 'hidden md:flex'}`}>
                            {/* Horizontal Mode Slider */}
                            <div className="flex items-center justify-around md:justify-around py-2 border-b border-black/5 dark:border-white/5 bg-surface relative overflow-x-auto no-scrollbar">
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`relative px-4 py-1.5 flex flex-col items-center gap-1 transition-all z-10 ${bookMode === mode.id ? 'text-foreground' : 'text-zinc-600 hover:text-zinc-600 dark:text-zinc-400'}`}
                                    >
                                        {bookMode === mode.id && (
                                            <motion.div 
                                                layoutId="activeBookMode"
                                                className="absolute inset-0 bg-black/5 dark:bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-lg shadow-[0_0_15px_rgba(255,255,255,0.05)]"
                                                transition={{ type: "spring", bounce: 0.2, duration: 0.6 }}
                                            />
                                        )}
                            ...
                            <div className="flex-1 flex flex-col min-w-0">
                                <div className="h-8 border-b border-black/5 dark:border-white/5 px-4 flex items-center justify-between shrink-0 bg-surface/50">
                                    <h3 className="text-[8px] uppercase font-black tracking-[0.2em] flex items-center gap-2 text-primary-600 dark:text-primary-500">
                                    ...
                                    <AnimatePresence mode="wait">
                                        {bookMode === 'book' && (
                                            ...
                                            >
                                                ...
                                                {/* Spread Section */}
                                                <div className="py-2 border-y border-black/5 dark:border-white/5 bg-surface flex flex-col items-center justify-center relative">
                                                    ...
                                                    {[
                                                        ...
                                                        <div key={i} className="group cursor-pointer">
                                                            <div className="flex items-center gap-2 mb-1">
                                                                ...
                                                                <span className="text-[7px] uppercase font-black tracking-widest text-zinc-600 dark:text-zinc-600 dark:text-zinc-400">{item.date}</span>
                                                            </div>
                                                            <p className="text-[11px] font-bold text-zinc-600 dark:text-zinc-400 group-hover:text-foreground transition-colors leading-relaxed">{item.title}</p>
                                                        </div>
                                                    ))}
                                                </div>
                                            </motion.div>
                                            ...
                                            >
                                                <div className="p-4 space-y-3">
                                                    {openOrders.length === 0 ? (
                                                        ...
                                                        <div key={order.id} className="p-4 bg-surface border border-black/5 dark:border-white/5 rounded-xl group hover:border-black/10 dark:border-white/10 transition-all">
                                                            <div className="flex justify-between items-start mb-3">
                                                                <div>
                                                                    ...
                                                                    <p className="text-[9px] text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 font-mono">QTY: {order.unfilled_quantity} / {order.quantity} BK</p>
                                                                </div>
                                                                <div className="flex items-center gap-1">
                                                                    <button 
                                                                        onClick={() => setModifyingOrder(order)}
                                                                        className="p-2 text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 hover:text-foreground hover:bg-black/5 dark:bg-black/5 dark:bg-white/5 rounded-lg transition-all"
                                                                        title="Modify"
                                                                    >
                                                                        <Edit2 size={14} />
                                                                    </button>
                                                                    <button 
                                                                        ...
                                                                        className="p-2 text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 hover:text-red-500 hover:bg-red-500/5 rounded-lg transition-all"
                                                                        title="Cancel"
                                                                    >
                                                                        <Trash2 size={14} />
                                                                    </button>
                                        ...
                                        {bookMode === 'vault' && (
                                            ...
                                            >
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div className="p-4 bg-surface border border-black/5 dark:border-white/5 rounded-xl">
                                                ...
                                                <div className="p-4 bg-surface border border-primary-500/20 rounded-xl relative overflow-hidden">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-6">
                                                        <div>
                                                            <p className="text-[7px] uppercase tracking-widest text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 mb-1">Quantity</p>
                                                            <p className="text-xs font-mono font-black text-foreground">{activeHoldingStats.quantity} BK</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-[7px] uppercase tracking-widest text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 mb-1">Unrealized P&L</p>
                                                    ...
                                                    {holdings.length === 0 ? (
                                                        ...
                                                        <div className="space-y-2">
                                                            {holdings.map((h, idx) => {
                                                                ...
                                                                return (
                                                                    <div key={idx} className="p-3 bg-surface border border-black/5 dark:border-white/5 rounded-lg flex items-center justify-between group hover:border-black/10 dark:border-white/10 transition-colors">
                                                                        <div className="min-w-0">
                                                                            <p className="text-[10px] font-bold text-foreground truncate">{proj?.title || 'Unknown Project'}</p>
                                                                            <p className="text-[8px] text-zinc-600 dark:text-zinc-600 dark:text-zinc-400 font-mono uppercase tracking-tighter">NODE: {h.project_id.substring(0,8)}...</p>
                                                                        </div>
                                                ...
                                                <div className="p-4 bg-surface border border-black/5 dark:border-white/5 rounded-xl space-y-4">
                                                    <p className="text-[8px] uppercase tracking-widest text-zinc-600 mb-2">Vault Performance</p>
                                                    <div className="flex items-center justify-between mb-2">
                                                        <span className="text-[10px] font-bold text-zinc-600 dark:text-zinc-400">Total</span>
                                                    ...
                                                    <div className="flex items-center justify-between px-1 pt-2">
                                                        <div className="flex flex-col">
                                                            ...
                                                            <span className="text-[11px] font-mono font-bold text-zinc-600 dark:text-zinc-400">₹{activeHoldingStats.avgPrice.toLocaleString()}</span>
                                                        </div>
                                        ...
                                        {bookMode === 'dao' && (
                                            ...
                                            >
                                                <div className="p-5 bg-primary-500/5 border border-primary-500/10 rounded-xl space-y-3">
                                                    ...
                                                    <p className="text-[10px] text-zinc-600 dark:text-zinc-400 leading-relaxed">Proposal #EX-240: Implement automated market-making for secondary pools.</p>
                                                ...
                                                <div className="space-y-4">
                                                     ...
                                                     <div className="flex items-center justify-between px-1">
                                                         <span className="text-[10px] text-zinc-600 dark:text-zinc-600 dark:text-zinc-400">Voting Power</span>
                                ...
                                <div className="hidden md:flex w-full md:w-80 lg:w-96 flex flex-col bg-background shrink-0 border-l border-black/5 dark:border-white/5 shadow-[-10px_0_30px_rgba(0,0,0,0.02)] dark:shadow-none relative z-10">
                                    {/* Trade Form */}
                                    <div className="p-5 md:p-6 border-b border-black/5 dark:border-white/5 bg-surface">
                                        <TradeForm 
                                            orderType={orderType}
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 p-4 bg-background/80 backdrop-blur-xl border-t border-black/5 dark:border-white/5 flex gap-3 z-[70]">
            <button 
