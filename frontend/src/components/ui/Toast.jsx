    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed bottom-24 md:bottom-8 md:right-8 left-4 right-4 md:left-auto z-[200] flex items-center gap-4 bg-background border border-border p-4 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] md:min-w-[380px] backdrop-blur-xl"
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] font-medium text-foreground/50 line-clamp-2 leading-relaxed">{message}</p>
          </div>
 
          <button 
            onClick={onClose}
            className="h-8 w-8 shrink-0 flex items-center justify-center hover:bg-foreground/5 rounded-full transition-all text-foreground/30 hover:text-foreground border border-transparent hover:border-border"
          >
            <X size={16} />
          </button>
        </motion.div>
